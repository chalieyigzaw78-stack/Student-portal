...
def staff_dashboard():
    ...
    recent   = db.execute("""
        ...
        WHERE r.staff_id=? ORDER BY r.created_at DESC LIMIT 20
    """, (user["id"],)).fetchall()
    db.close()
...
def staff_view_student(sid):
    ...
    results = db.execute("""
        SELECT r.*, s.name as subject_name, s.code as subject_code, s.credit_hours
        FROM results r JOIN subjects s ON r.subject_id=s.id
        WHERE r.student_id=? ORDER BY r.academic_year DESC, r.semester DESC, s.name
    """, (sid,)).fetchall()
    db.close()
    semesters = {}
    for r in results:
...
def admin_add_subject():
    ...
    db   = get_db()
    try:
        db.execute("INSERT INTO subjects (code,name,credit_hours,department) VALUES (?,?,?,?)",
